...
from app.proctoring.identity import IdentityUnavailableError
...
@pytest.mark.asyncio
async def test_analyze_camera_frame_unparseable_json_degrades_gracefully():
    """A reasoning VLM returning prose must not surface as a client 400."""
    with patch(
        "app.proctoring.vlm_face.acompletion_vision_json",
        new_callable=AsyncMock,
        side_effect=json.JSONDecodeError("no json", "The user wants me to...", 0),
    ):
        with pytest.raises(IdentityUnavailableError):
            await analyze_camera_frame("aGVsbG8=", match_threshold=0.7)
...
def test_violations_from_looking_away():
    analysis = CameraAnalysisResult(
        face_visible=True,
        person_count=1,
        face_count=1,
    ...
    look_away = next(v for v in violations if v.event_type == "looking_away")
    assert look_away.description == "Learner not facing the screen"
