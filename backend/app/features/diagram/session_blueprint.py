"""Resolve blueprint limits for a diagram session."""
...
async def diagram_blueprint_context(
    db: AsyncSession,
    session_id: str,
    ...
    total = tool_question_count(blueprint, "diagram", legacy_keys=("diagram",)) or 3
    profile = _parse_profile(session.learner_profile_json)
    return session, blueprint, max(1, total), profile


__all__ = ["diagram_blueprint_context"]
