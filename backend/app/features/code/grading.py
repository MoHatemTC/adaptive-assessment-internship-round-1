...
_RUBRIC_SYSTEM_PROMPT = (
    ...
    "submission on two qualitative dimensions: 'approach' (clarity, "
    "correctness of strategy, edge-case handling) and 'efficiency' "
    "(algorithmic complexity, avoidance of needless work). Do NOT judge raw "
    "test pass rate — that is measured separately. Respond with ONLY valid "
    "JSON of the form: "
    '{"dimensions": [{"name": "approach", "score": 0.0-1.0, "feedback": "..."}, '
    '{"name": "efficiency", "score": 0.0-1.0, "feedback": "..."}], '
    '"overall": 0.0-1.0}. Scores are floats in [0,1].'
)
...
async def _grade_with_llm(
    *,
    ...
    Isolated so tests can substitute a deterministic result without a live
    model call.
    ...
    return RubricScores.model_validate_json(_extract_json(content))
