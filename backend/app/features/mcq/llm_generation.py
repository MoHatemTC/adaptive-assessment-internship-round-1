...
Generates the next MCQ via the platform LiteLLM gateway
(:func:`app.core.llm.get_llm_with_tracing`) and stores it in the MCQ tables
before returning. Salvaged from the ``mcq-tool`` branch (credited to Malak) and
aligned to the unified schema:

- ``mcq_questions`` stores ``difficulty`` and ``dimension``.
- ``dimension`` values are the platform vocabulary
  ``thinking / soft / work / digital_ai / growth`` so the shared adaptation
  agent can read ``question.dimension.value``.
...
def _build_mcq_generation_prompt(
    ...
    Args:
        next_plan: Adaptive plan context (target dimension/focus/difficulty).
        learner_profile: Optional learner context for the generator.
        admin_config: Optional admin/blueprint configuration.
...
def _validate_generated_mcq(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Args:
        data: The parsed MCQ dict from the LLM.

    Returns:
        A normalized MCQ dict with cleaned options, difficulty, and dimension.

    Raises:
        ValueError: If required fields are missing or the options are malformed.
    """
    required_fields = [
        "question_text",
        "difficulty",
...
async def generate_and_store_next_mcq(
    ...
    """Generate the next MCQ via LiteLLM and store it in the database.
    ...
    Returns:
        The created question serialized without the correct answer (includes
        ``id``, ``question_text``, ``difficulty``, ``dimension``, ``options``).

    Raises:
        Exception: Propagates any LLM invocation error after recording metrics.
    """
